...
package edu.udo.scaffoldhunter.plugins.datacalculation.impl.estatebitstringfingerprint;
...
public class EStateBitStringCalcPlugin extends AbstractCalcPlugin {

    @Override
    public String getTitle() {
        return "EStateBitStringFingerprinter";
    }

    @Override
    public String getID() {
        return "CDK.EStateBitStringFingerprinter_1.0";
    }
    ...
    public PluginSettingsPanel getSettingsPanel(Serializable settings, Object arguments) {
        Preconditions.checkArgument(arguments instanceof EStateBitStringCalcPluginArguments || arguments == null);

        if (arguments == null) {
            arguments = new EStateBitStringCalcPluginArguments();
        }

        return new EStateBitStringCalcPluginSettingsPanel((EStateBitStringCalcPluginArguments) arguments);
    }

    @Override
    public CalcPluginResults getResults(Object arguments, Iterable<IAtomContainer> molecules, MessageListener msgListener) {
        Preconditions.checkArgument(arguments instanceof EStateBitStringCalcPluginArguments);
        Preconditions.checkNotNull(molecules);
        Preconditions.checkNotNull(msgListener);

        return new EStateBitStringCalcPluginResults(Iterables.transform(molecules, new CalcPluginMoleculeTransformFunction(
                (EStateBitStringCalcPluginArguments) arguments, msgListener)), msgListener);
    }
}
