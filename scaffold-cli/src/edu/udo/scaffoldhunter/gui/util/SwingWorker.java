...
// EDIT: we use a local copy of AccumulativeRunnable, since this version isn't accessible
...
//    private volatile int progress;
    ...
    private boolean restartingWorker = false;
...
//    private AccumulativeRunnable<Integer> doNotifyProgressChange;
    ...
    /**
     * Initializes the worker. After calling this method the worker can be
     * started again. The worker should be stopped first, since otherwise
     * methods that call future, such as {@link #get()}, would use the last
     * instance and the results of the first instance would be lost.
     */
    protected void reset() {
        ...
        future = new FutureTask<T>(callable) {
            ...
            public void run() {
                try {
                    super.run();
                } catch (SwingWorker.StopException e) {
                }
            }
            @Override
            protected void done() {
     ...
     * @throws EDIT: nothing! was: IllegalArgumentException is value not from 0 to 100
     */
//    protected final void setProgress(int progress) {
        // EDIT: commented out this crap
...
//    }
    ...
    /**
     * Stops the worker and starts it again.
     */
    public final void restart() {
        restartingWorker = true;
        cancel(true);
        // XXX wait until restartingWorker is false?
        reset();
        execute();
    }
    ...
    /**
     * Indicates whether the worker currently is being restarted. Can be used in
     * subclasses' {@link #done()} method to find out if it's being called from
     * within {@link #cancel(boolean)} while the worker is being stopped and
     * started again. In this case the worker's listeners shouldn't be notified
     * about the forced termination of the worker.
     * 
     * @return true iff the worker is being started again
     */
    protected boolean isRestartingWorker() {
        return restartingWorker;
    }
    ...
    /**
     * Schedules a runnable for execution on the event dispatcher thread.
     * 
     * Use this method instead of {@link SwingUtilities#invokeLater(Runnable)}
     * if the order of execution with respect to the build-in messages is
     * important. {@link SwingUtilities#invokeLater(Runnable)} also guarantees
     * the order if execution, but since the internal messages use a timer-based
     * system, the custom messages may be executed before or after the built-in
     * messages if scheduled via invokeLater.
     */
    ...
    /**
     * can be thrown in subclasses to stop the execution of the worker 
