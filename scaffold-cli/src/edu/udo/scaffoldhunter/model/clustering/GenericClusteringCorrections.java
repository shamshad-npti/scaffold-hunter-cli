 ...
 * handle Linkages that do not fulfill the reducibility property!
 * 
 ...
 * 
 */
public class GenericClusteringCorrections<S extends Structure> implements HierarchicalClustering<S> {
    private static Logger logger = LoggerFactory.getLogger(GenericClusteringCorrections.class);
     ...
     */
    public GenericClusteringCorrections(NNSearch<S> nnSearch) {
    ...
    public HierarchicalClusterNode<S> calc() throws ClusteringException {
        ...
        /*
         * Node -> NN(node), Dist(Node, NN(Node))
         */
        HashMap<HierarchicalClusterNode<S>, NodeAndDist<S>> nearestNeighbour = new HashMap<HierarchicalClusterNode<S>, NodeAndDist<S>>(
                initialSize);
        ...
        while (nnSearch.size() > 1) {
            ...
            NodeAndDist<S> nn = nearestNeighbour.get(minNode);
             ...
             */
            while (!nnSearch.currentLevelContains(minNode) || !nnSearch.currentLevelContains(nn.node)) {
                ...
                if (nnSearch.currentLevelContains(minNode)) {
                    ...
                    if (newNnAndDist != null) {
                        ...
                        nearestNeighbour.put(minNode, new NodeAndDist<S>(newNn, newDist));
            ...
            /*
             * check if nn.node and minNode are reciprocal and if the reverse
             * distance is greater than the distance itself.
             * 
             * If so we can make the distance symmetric and correct this
             * heuristic error.
             * 
             * -> reinsert in Q with increased distance to not distract the
             * merging process
             */
            NodeAndDist<S> nnOfNn = nearestNeighbour.get(nn.node);
            if (nnOfNn.node.equals(minNode) && nn.dist < nnOfNn.dist) {
                nn.dist = nnOfNn.dist;
                Q.insert(nnOfNn.dist, minNode);
                continue;
            }

            // Merge Clusters
            logger.debug("Merge Nodes: {} ---- {}", minNode, nn.node);
            HierarchicalClusterNode<S> newMergedCluster = nnSearch.merge(minNode, nn.node);
            ...
            nearestNeighbour.remove(nn.node);
     ...
     */
    private void initNode(HashMap<HierarchicalClusterNode<S>, NodeAndDist<S>> nearestNeighbour,
        ...
        if (nnAndDist != null) {
            ...
            Double nnDistance = nnAndDist.getValue();
            nearestNeighbour.put(node, new NodeAndDist<S>(nn, nnAndDist.getValue()));
            Q.insert(nnDistance, node);
        } else {
